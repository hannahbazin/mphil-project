...
libs_combined_filt <- RunPCA(libs_combined, npcs = 30)

```

## Determine dimensionality of the dataset

We plot an elbow plot to determine the dimensionality of the dataset. From this plot we can see that the line starts to flatten significantly around PC 10. Therefore in the downstream analysis we keep 15 dimensions of the dataset.
...
libs_combined_filt <- RunUMAP(libs_combined_filt, reduction = "pca", dims = 1:15)
...
I adapted the resolution to create an approximate number of clusters I am expecting based on the paper of reference (Angueira et al., 2021) - i.e. around 11 clusters.

```{r}

libs_combined_filt <- FindNeighbors(libs_combined_filt, reduction = "pca", dims = 1:15)

libs_combined_filt <- FindClusters(libs_combined_filt, resolution = 0.35, random.seed = 1996)
...
if(file.exists("data/analysis/cluster_markers.RData")){
  ...
  load(file = "data/analysis/cluster_markers.RData")
    ...
    markers <- FindAllMarkers(libs_combined_filt, only.pos = T, min.pct = 0.3, logfc.threshold = 0.3)
...
DotPlot(libs_combined_filt,
  ...
  ggtitle("Early marker expression")
...
DotPlot(libs_combined_filt,
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  ggtitle("Intermediate marker expression")
...
This dot plot contains the genes I found were important marker genes for the cell types I identified in my clusters.
...
FeaturePlot(libs_combined_filt, features = c(
...
To show proper integration of all three samples
...
The dot size represents the percent expressed, i.e. how many cells in the given cluster express the gene. The dot colour shows the average expression, i.e. the expression level in cells that do express the gene.
...
markers_to_plot <- c(
          ...
          # Pre-adipocytes
          "NEGR1", "EBF2", "DCN", "MGP", "PDGFRA", "FBLN1", "COL1A1", "COL1A2", "FSTL1", "CREB5", "PPARG",
          # Mature brown adipocytes
...
FeaturePlot(libs_combined_filt,
            features = c(
...
TODO: MAKE LEGEND TEXT BIGGER AND BE IN TOP RIGHT CORNER. AND MAKE FEATURE PLOTS THESIS READY.
...
# Plot violin plot of interesting genes

VlnPlot(libs_combined_filt,
                  features = c("EBF2", "PPARG", "PDGFRA", "ADIPOQ", "MYH11", "PDGFRB"),
                  cols = namedclusterColours,
                  ncol = 3,
                  pt.size = 0)

violin <- VlnPlot(libs_combined_filt,
                  features = c("EBF2", "PPARG", "PDGFRA", "ADIPOQ", "MYH11", "PDGFRB"),
                  cols = namedclusterColours,
                  ncol = 3,
                  pt.size = 0)) +
  ...
  #ggtitle("Violin Plots of Marker Genes Across PVAT Cell Types (snRNA-seq)") + 
