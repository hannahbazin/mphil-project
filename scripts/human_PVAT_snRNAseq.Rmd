...
dim_heatmap <- wrap_plots(individual_heatmaps, ncol = 3, guides = "collect") &
  theme(
    axis.text.y = element_text(size = 9, hjust = 1.0, vjust = 0.5),
    axis.text = element_text(size = 8),
...
# Combine panels A and B with proper tagging
panel_A <- wrap_elements(dim_heatmap) + labs(tag = "A")
panel_B <- variance_plot + labs(tag = "B")

# Combine panels A and B
combined_pca_figure <- panel_A / panel_B + 
...
pdf("results/humanPVATsn/final_figs/combined_pca_analysis.pdf", width = 14, height = 21)
...
elbow_plot <- ElbowPlot(libs_combined_filt, ndims = 30) + 
  ...
  annotate("text", x = 19, y = max(libs_combined_filt[["pca"]]@stdev[1:30]) * 0.8, 
...
variance_plot <- ggplot(variance_df, aes(x = factor(PC, levels = PC))) +
 ...
 geom_text(aes(y = Variance, label = paste0(round(Variance, 1), "%")), 
           vjust = -0.3, size = 3) +
 geom_text(aes(y = Cumulative, label = paste0(round(Cumulative, 1), "%")), 
           vjust = -2.0, hjust = 0.75, color = "red", size = 3) +
 labs(x = "Principal component", 
      y = "Variance explained (%)") +
 theme_minimal() +
 theme(
...
dot_plot <- DotPlot(anno_combined2, features = markers_to_plot, dot.scale = 7, cols = "PiYG") +
